...
pub fn gen_endpoint_info(device_type: u8, device_name: &str) -> Vec<u8> {
...
pub fn gen_fef3_fast_advertisement(endpoint_id: [u8; 4], endpoint_info: &[u8]) -> Vec<u8> {
    let mut body = Vec::new();
    let vers_pcp = (1 << 5) | 3; // version=1, pcp=3 -> 0x23
    body.push(vers_pcp);
    ...
    hasher.update(&body);
    let device_token = &hasher.finalize()[0..2];

    let mut out = Vec::new();
    out.push(0x4a); // FRAME_TYPE_FAST_ADVERTISEMENT
...
pub fn gen_fe2c_pulse(endpoint_id: [u8; 4]) -> Vec<u8> {
    let mut out = Vec::new();
    out.extend_from_slice(&[252, 18, 142, 0, 66, 0, 0, 0, 0, 0, 0, 0, 0, 0]);
    let salt = rand::rng().random::<u8>();
    out.push(salt);
    use sha2::Digest;
    let mut hasher = sha2::Sha256::new();
    hasher.update(&endpoint_id);
    let hash = hasher.finalize();
    out.extend_from_slice(&hash[0..8]);
    out
}
...
mod tests {
    ...
    fn test_gen_and_parse_mdns_info() {
        ...
        let info_raw = gen_endpoint_info(device_type.clone() as u8, device_name);
        let info = base64::engine::general_purpose::URL_SAFE_NO_PAD.encode(&info_raw);
